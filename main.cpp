...
int main() {
  ...
  int pageSize = 16384;
  void *traceeStack =
      (uint8_t *)mmap(NULL, pageSize, PROT_WRITE | PROT_READ,
  ...
  for (;;) {
    ...
    int ret = system(
        "objdump -b binary -m Loongarch64 -D temp | grep word >/dev/null");
    ...
    // perror("ptrace");
    // write instruction
    // memcpy(data->instructionPointer, &data->currentInstruction,
    // instructionSize); clearCache(data->instructionPointer,
    // data->instructionPointer + instructionSize);
    ...
    ptrace(PTRACE_GETREGSET, pid, NT_PRSTATUS,
           &resultIovec); // gets resulting state
                          //  perror("ptrace");
